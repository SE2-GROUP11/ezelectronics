...
import handleError from "../errors/genericErrorHandler";
...
class ProductRoutes {
	...
	initRoutes() {
		...
		this.router.post(
			...
			body("model").exists().notEmpty().isString(),
			body("category").exists().notEmpty().isString().isIn(["Smartphone", "Laptop", "Appliance"]),
			body("quantity")
				.exists()
				.notEmpty()
				.isInt()
		...
		this.router.patch(
			...
			this.authenticator.isLoggedIn,
			this.authenticator.isAdminOrManager,
			param("model").exists().notEmpty().isString(),
			body("quantity")
				.exists()
				.notEmpty()
				.isInt()
		...
		this.router.get(
			...
			query("grouping")
				...
				.custom((val, { req }) => {
					...
					else if (val === "model") return req.query.model && req.query.category === undefined;
					else return req.query.category === undefined && req.query.model === undefined;
				}),
			query("category")
				.optional()
				.isString()
				...
				.custom((val, { req }) => {
					if (!req.query.grouping && val) return false;
					return true;
				}),
			query("model")
				.optional()
		...
		this.router.get(
			...
			(req: any, res: any, next: any) => {
				...
				if (req.modelNotFoundError) {
					return res.status(404).send("Model does not exist");
				}
				this.controller
		...
		this.router.delete(
			...
			(req: any, res: any, next: any) =>
				this.controller
					.deleteAllProducts()
					.then(() => res.status(200).end())
					.catch((err: any) => handleError(res, err, next))
		);
