...
class UserRoutes {
	...
	initRoutes() {
		...
		this.router.post(
			...
			body("surname").exists().isString().notEmpty(),
			body("password").exists().isString().notEmpty(),
			body("role").exists().isString().notEmpty().matches(new RegExp("(Manager)|(Customer)|(Admin)")),
		 ...
		 */
		this.router.get("/", this.authService.isLoggedIn, this.authService.isAdmin, (req: any, res: any, next: any) =>
			this.controller
				.getUsers()
		...
		this.router.get(
			...
			param("role").exists().isString().notEmpty().matches(new RegExp("(Manager)|(Customer)|(Admin)")),
			this.errorHandler.validateRequest,
			(req: any, res: any, next: any) =>
				this.controller
		...
		this.router.delete(
			...
			(req: any, res: any, next: any) =>
				this.controller
					...
					.then((done) => (done ? res.status(200).end() : handleError(res, new UserNotFoundError(), next)))
					.catch((err) => handleError(res, err, next))
		);
